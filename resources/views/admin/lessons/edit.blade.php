...
@section('content')
<h1>Редактировать урок: {{ $lesson->title }}</h1>
...
<form action="{{ route('admin.lessons.update', [$course, $lesson]) }}" method="POST">
    ...
    <div class="form-group">
        ...
        <input type="text" name="title" value="{{ old('title', $lesson->title) }}" class="input-field @error('title') is-invalid @enderror">
        @error('title') <div class="error-text">{{ $message }}</div> @enderror
    </div>

    <div class="form-group">
        <label>Описание</label>
        <textarea name="description" class="input-field @error('description') is-invalid @enderror">{{ old('description', $lesson->description) }}</textarea>
        @error('description') <div class="error-text">{{ $message }}</div> @enderror
    </div>

    <div class="form-group">
        <label>Ссылка на видео (SuperTube)</label>
        <input type="url" name="video_link" value="{{ old('video_link', $lesson->video_link) }}" placeholder="https://super-tube.cc/video/..." class="input-field @error('video_link') is-invalid @enderror">
